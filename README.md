<h1 align="center">Joe Njenga</h1>

<p align="center">
  <strong>AI Engineer | AI Researcher | Automation Builder | Technical Writer | Educator</strong>
</p>

<p align="center">
  I build, test, review, and explain modern AI systems, from frontier models and agentic coding tools to production automation workflows.
</p>

<p align="center">
  <a href="https://medium.com/@joe.njenga"><img src="https://img.shields.io/badge/Medium-23K%2B%20Followers-000000?style=for-the-badge&logo=medium&logoColor=white" alt="Medium"></a>
  <a href="https://newsletter.claudecodemasterclass.com/"><img src="https://img.shields.io/badge/Newsletter-70K%2B%20Subscribers-FF6719?style=for-the-badge&logo=substack&logoColor=white" alt="Newsletter"></a>
  <a href="https://github.com/Njengah/claude-code-cheat-sheet"><img src="https://img.shields.io/github/stars/Njengah/claude-code-cheat-sheet?style=for-the-badge&logo=github&label=Claude%20Code%20Cheat%20Sheet" alt="Claude Code Cheat Sheet stars"></a>
  <a href="https://njengah.com"><img src="https://img.shields.io/badge/Website-njengah.com-2563EB?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website"></a>
  <a href="https://njengah.com/contact"><img src="https://img.shields.io/badge/Work%20With%20Me-Contact-16A34A?style=for-the-badge" alt="Contact"></a>
</p>

---

## Start Here

These are the best entry points into my current AI work:

| Resource | Description |
| --- | --- |
| [400+ AI tools, testing reviews, and tutorials](https://medium.com/@joe.njenga) | My public research log for model releases, AI coding tools, automation workflows, and AI engineering patterns |
| [Claude Code Cheat Sheet](https://github.com/Njengah/claude-code-cheat-sheet) | A practical Claude Code reference covering commands, workflows, subagents, automation, and power-user usage |
| [Claude Code Tutorials](https://github.com/Njengah/claude-code-tutorials) | Practical demos and use cases for learning Claude Code through examples |
| [AutoWP MCP](https://github.com/Njengah/autowpmcp) | A real MCP server that connects Claude to WordPress publishing workflows |
| [Claude Code Masterclass](https://newsletter.claudecodemasterclass.com/) | My newsletter and course ecosystem for Claude Code, Codex, agents, and AI-native development |

##  AI Engineering

I am focused on the practical discipline of AI engineering: building intelligent software systems that combine LLMs, agents, tools, automations, integrations, evaluations, and human workflows.

Over the last year, I have been building public authority in this space by testing new AI tools, publishing deep technical content, reviewing industry trends, and teaching developers how to use AI systems in real work.

| Platform / Skill | Content / Audience |
| --- | --- |
| Technical writing | 400+ articles on AI, software engineering, automation, AI coding, and developer tooling |
| Medium audience | 23K+ followers across AI and software engineering content |
| Newsletter | 70K+ subscribers on Claude Code Masterclass |
| YouTube | Built a channel around AI tools, model releases, workflows, and practical AI engineering |
| Product education | Building a Claude Code, Codex, agents, and AI engineering course ecosystem |
| Industry coverage | Reviewing major AI coding tools, model releases, AI workflow platforms, and emerging trends |

## Portfolio 

My GitHub portfolio is built around one practical argument:

> The best AI engineers do not only talk about models. They test tools, build workflows, publish working examples, evaluate failure modes, and turn lessons into reusable systems.

That is the direction of this profile. My most significant AI tutorials, Medium articles, newsletter lessons, and YouTube reviews become the source material for public testing projects.

```text
Tutorial -> Working repo -> Test workflow -> Review notes -> Article/video/course lesson
```

This creates a portfolio that is not based on promises. It is based on repeated public experiments.

## I Study, Test, & Build

I actively test and document the fast-moving AI engineering stack, including:

- Frontier model releases from OpenAI, Anthropic, Google, DeepSeek, Qwen, and other major labs.
- AI coding tools including Claude Code, Codex, Cursor, Windsurf-style IDEs, agentic coding platforms, and terminal-first AI workflows.
- Emerging AI developer tools, open-source agent frameworks, automation platforms, and trends such as OpenClaw and similar industry movements.
- MCP servers, tool use, subagents, hooks, project instructions, coding manifests, and AI-native development environments.
- Practical automation systems that connect AI models to APIs, documents, data sources, content workflows, and business operations.

My goal is not just to react to new tools. I study how they change software development, developer productivity, product building, technical education, and the future of AI-native work.

## Core Testing Projects

These are the projects that currently anchor my portfolio:

| Project | What It Proves | Status |
| --- | --- | --- |
| [Claude Code Cheat Sheet](https://github.com/Njengah/claude-code-cheat-sheet) | I test Claude Code deeply enough to document practical usage from beginner commands to advanced workflows, automation, subagents, and integrations | Public |
| [Claude Code Tutorials](https://github.com/Njengah/claude-code-tutorials) | I turn Claude Code features into practical demos, examples, and use cases developers can learn from | Public |
| [AutoWP MCP](https://github.com/Njengah/autowpmcp) | I can build real MCP integrations that connect Claude to external platforms, in this case WordPress content publishing | Public |
| AI Tool Review Bench | A structured benchmark system for comparing Claude Code, Codex, Cursor, OpenClaw, model releases, and AI coding tools | Building |
| Model Release Test Log | Repeatable tests for coding, reasoning, refactoring, debugging, agent behavior, context handling, and workflow reliability | Building |
| AI Tutorial Test Projects | Repos generated from my most popular Medium tutorials and newsletter masterpieces, each with code, prompts, workflows, notes, and results | Building |

## Tutorials

My strongest tutorials and newsletter essays become portfolio projects with a consistent structure:

| Section | Purpose |
| --- | --- |
| Problem | What the tutorial, tool, model, or workflow is trying to solve |
| Setup | Exact environment, dependencies, prompts, config, and assumptions |
| Implementation | Working code, commands, screenshots, and examples |
| Test Cases | Repeatable tasks used to evaluate the tool or workflow |
| Results | What worked, what failed, and where the tool is strong or weak |
| Engineering Notes | Lessons for real AI engineering, not just surface-level tool usage |
| Content Links | Related Medium article, newsletter issue, video, and follow-up notes |

This is how I want the profile to evolve: every major article becomes a working artifact, and every working artifact feeds back into better writing, videos, and course material.

## Current Focus Areas

```text
AI Engineering        -> LLM apps, agents, evals, retrieval, tool use, workflow orchestration
AI Research           -> Translating papers, model releases, and benchmarks into practical engineering patterns
AI Automation         -> Business workflows, content systems, research pipelines, and internal tools
Agentic Coding        -> Claude Code, Codex, Cursor, MCP, subagents, hooks, and AI-native repos
Developer Education   -> Articles, newsletters, YouTube, courses, playbooks, and project-based learning
```

## My Testing

 I test, write about, review, and teach.

| Project Direction | What It Will Demonstrate |
| --- | --- |
| AI Agent Starter Kit | Tool use, memory, planning, evals, traces, permissions, and production-ready agent structure |
| MCP Server Lab | Real MCP servers for connecting coding agents to APIs, databases, files, analytics, and publishing workflows |
| Claude Code Workflow Library | Reusable commands, subagents, hooks, project instructions, and workflow recipes |
| AI Automation Workbench | End-to-end automations for content, research, operations, newsletters, and business systems |
| LLM App Patterns | Reference apps for RAG, structured outputs, function calling, multimodal inputs, and human-in-the-loop flows |
| AI Tool Review Bench | A structured way to compare AI coding tools, model releases, IDE agents, automation platforms, and emerging tools |
| Research To Product Notes | Short repos translating AI research and industry trends into buildable product ideas |

## Content Platforms

I create across multiple channels so each idea can become a written guide, a video walkthrough, a newsletter issue, a course lesson, and a working repo.

- **Medium:** long-form AI engineering articles, tutorials, tool reviews, and technical essays.
- **Claude Code Masterclass Newsletter:** practical Claude Code, Codex, agentic coding, and AI workflow education.
- **YouTube:** demos, walkthroughs, model tests, coding tool reviews, and AI engineering explainers.
- **GitHub:** reference implementations, portfolio projects, automation labs, and reusable developer workflows.
- **Courses:** structured learning paths for developers who want to become effective AI engineers.

## Technical Stack

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript">
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" alt="React">
  <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white" alt="Next.js">
  <img src="https://img.shields.io/badge/Vue-4FC08D?style=flat-square&logo=vuedotjs&logoColor=white" alt="Vue">
  <img src="https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white" alt="OpenAI">
  <img src="https://img.shields.io/badge/Claude-D97757?style=flat-square&logo=anthropic&logoColor=white" alt="Claude">
  <img src="https://img.shields.io/badge/Gemini-4285F4?style=flat-square&logo=googlegemini&logoColor=white" alt="Gemini">
  <img src="https://img.shields.io/badge/Cursor-000000?style=flat-square&logo=cursor&logoColor=white" alt="Cursor">
</p>

I work with Python, JavaScript, TypeScript, React, Next.js, Vue, REST APIs, GraphQL, webhooks, LLM APIs, AI agents, MCP servers, retrieval systems, structured outputs, evaluation workflows, and automation infrastructure.

## How I Think About AI Engineering

AI engineering is not just prompting. It is the engineering work required to make AI systems reliable, useful, measurable, and integrated into real workflows.

That means building around:

- Model selection and model behavior.
- Tool use and external system integration.
- Memory, retrieval, and context engineering.
- Evaluation, observability, and failure analysis.
- Human review, permissions, and safety boundaries.
- Product workflows that make AI useful to real users.

## Collaboration

I am open to collaborations around AI engineering, automation, developer tooling, technical education, research translation, and agentic software systems.

If you are building with AI and need help turning ideas into working systems, reach me through [njengah.com/contact](https://njengah.com/contact).
